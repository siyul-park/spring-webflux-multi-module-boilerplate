...
class ClientTest : ClientTestHelper() {
    ...
    fun grant() = blocking {
        ...
        coVerify(exactly = 1) { clientScopeEntityRepository.findAll(any()) }
        ...
        coVerify(exactly = 2) { clientScopeEntityRepository.findAll(any()) }
    }
    ...
    fun getScope() = blocking {
        ...
        coVerify(exactly = 3) { clientScopeEntityRepository.findAll(any()) }
        ...
        coVerify(exactly = 6) { clientScopeEntityRepository.findAll(any()) }

        loaded1?.getScope(deep = false)?.toSet()
        loaded1?.getScope(deep = false)?.toSet()

        coVerify(exactly = 8) { clientScopeEntityRepository.findAll(any()) }

        loaded2?.getScope(deep = false)?.toSet()
        loaded2?.getScope(deep = false)?.toSet()

        coVerify(exactly = 9) { clientScopeEntityRepository.findAll(any()) }
    }
