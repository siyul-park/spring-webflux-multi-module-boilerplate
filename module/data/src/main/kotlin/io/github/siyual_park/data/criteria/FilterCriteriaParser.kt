...
class FilterCriteriaParser<T : Any>(
    ...
    private fun parse(criteria: Criteria.Equals): (T) -> Boolean {
        return { property(criteria.key).get(it)?.equals(criteria.value) == true }
    }
    private fun parse(criteria: Criteria.NotEquals): (T) -> Boolean {
        return { property(criteria.key).get(it)?.equals(criteria.value) == false }
    }

    private fun parse(criteria: Criteria.Between): (T) -> Boolean {
        return {
            property(criteria.key).get(it)?.let { value ->
    ...
    private fun parse(criteria: Criteria.IsNull): (T) -> Boolean {
        return { property(criteria.key).get(it) == null }
    }
    ...
    private fun parse(criteria: Criteria.Like): (T) -> Boolean {
        ...
        return {
            property(criteria.key).get(it)?.let {
    ...
    private fun parse(criteria: Criteria.In): (T) -> Boolean {
        return { property(criteria.key).get(it)?.let { criteria.value.contains(it) } ?: false }
    }
    ...
    private fun parse(criteria: Criteria.IsTrue): (T) -> Boolean {
        return { property(criteria.key).get(it) == true }
    }
    private fun parse(criteria: Criteria.IsFalse): (T) -> Boolean {
        return { property(criteria.key).get(it) == false }
    }

    private fun property(key: String): KProperty1<T, *> {
        return clazz.memberProperties.find { it.name == key } ?: throw RuntimeException()
    }
}
